...
const [features, setFeatures] = useState(null);
  const getNameOfFeatured = () => {
    ...
    //   if(programmers[i].id === features){
    //     return programmers[i].name;
    //   }
    // }
// short cute way with .find
    const foundDev = programmers.find(dev => dev.id === features);
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: features ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setFeatures(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
